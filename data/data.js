...
// ── PROMPT_SYSTEM ──
/* 의뢰서 항목별 작성 지침 — 🤖 AI 버튼이 항목마다 다른 요구를 하게 한다.
   기획서를 옮겨 적는 데 그치지 않도록, ① 기획 단계 내용을 논거의 출발점으로만 쓰고
   앱이 확정한 수치·법령 근거를 결합해 심사 관점으로 다시 쓰게 한다.
   여기에 없는 항목은 지금까지와 같이 공통 지침만 적용된다. */
var DRAFT_ITEM_GUIDE = {
  '1-1':
    '[출발점]\n'
  + '- ① 기획 단계의 「추진 배경 및 필요성」·「사업 목표」를 논거의 출발점으로 삼는다.\n'
  + '- 기획서는 내부 기안용이고 의뢰서는 심사기관 제출용이다. 문장을 옮기지 말고,\n'
  + '  심사위원이 재정 투입의 타당성을 판단할 수 있도록 다시 쓴다.\n'
  + '[반드시 더할 것]\n'
  + '- 심사기관과 근거 조문을 본문에 명시한다.\n'
  + '- 총사업비·연면적 등 확정 수치를 문장 안의 논거로 인용한다(표로 나열하지 않는다).\n'
  + '- "왜 지금인가"(시급성)와 "왜 이 규모인가"(규모 적정성)를 각각 한 문장 이상 쓴다.\n'
  + '- 상위계획 정합성을 밝히되, 값이 없으면 [담당자 입력 필요: 중기지방재정계획 반영 연도·금액]으로 남긴다.\n'
  + '[분량·중복]\n'
  + '- 전체 5~8문장. 필요성의 상세 논증은 5-1 항목에서 다루므로 여기서는 개요 수준으로 쓴다.\n'
  + '[금지]\n'
  + '- ① 기획 단계에 없는 배경·목표를 새로 만들지 않는다.\n'
  + '- ① 기획 단계 내용과 사실관계가 어긋나지 않게 한다.\n'
  + '- 제공되지 않은 수치를 지어내지 않는다. 없으면 [담당자 입력 필요: 항목명]으로 표시한다.',
  '1-9':
    '[출발점]\n'
  + '- ① 기획 단계의 「기대효과」를 출발점으로 삼되, 정성적 서술에 머물지 않는다.\n'
  + '[반드시 더할 것]\n'
  + '- 정성적 효과를 정량 지표로 바꾼다 — 수혜인구(파주시 인구 530,000명 기준),\n'
  + '  B/C·NPV·IRR 등 위 컨텍스트에 제공된 값이 있으면 반드시 인용한다.\n'
  + '- 건설 단계 효과와 운영 단계 효과를 나누어 쓴다.\n'
  + '- 각 수치가 어떤 값에서 나왔는지 산정 근거를 함께 밝힌다.\n'
  + '[분량]\n'
  + '- 전체 5~8문장.\n'
  + '[금지]\n'
  + '- ① 기획 단계에 없는 효과를 새로 주장하지 않는다.\n'
  + '- 근거 없는 수치를 쓰지 않는다. 없으면 [담당자 입력 필요: 수치 항목명]으로 표시한다.'
};
